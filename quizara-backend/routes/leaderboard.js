...
router.route("/").post(verifyToken, async (req, res) => {
  ...
  try {
    ...
    const updatedGameArray = isgamesPlayed
      ? user.gamesPlayed.map((e) =>
          e.name == name ? { _id: e._id, name: e.name, totalScore } : e
        )
      : [...user.gamesPlayed, { name, totalScore }];

    let updatedUser = await extend(user, {
      ...user,
      gamesPlayed: updatedGameArray,
    });
